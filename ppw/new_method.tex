...
By integrating the linearized subthreshold current $I_{sub}$ in \eqref{eq:lin_leakage} and substituting $v$ with $f$ in \eqref{eq:f_v} into \eqref{eq:min_ppw}, we have
...
\begin{split}
\frac{1}{\text{PPW}} =  &\frac{p_{d}+(p_{d}^{-1/3}\cdot \frac{V_{max}-V_{th}}{f_{max}}+V_{th})(a_{s}(T_{i}+T_{a})+p_{0})}{p_{d}^{-1/3}}\\
=&p_{d}^{2/3}+(\frac{V_{max}-V_{th}}{f_{max}}+V_{th}\cdot p_{d}^{-1/3})(a_{s}(T_{i}+T_{a})+p_{0})
\end{split}
\end{equation}

We can see that in \eqref{eq:1_ppw_step2}, aside from $p_{d}$, the core's temperature rise $T_{i}$ is also unknown. However, we can express $T_{i}$ using $p_{d}$, as shown next.
